...
const Header = () => {
    ...
    return (
        <header className={clsx(
            ...
            scrolled ? "md:-translate-y-2" : "md:translate-y-0"
        )}>
            <div className={clsx(
                                        ...
                                        <div className="bg-white rounded-2xl shadow-premium border border-gray-50 overflow-hidden py-3">
                                            {link.submenu.map((subItem) => (
                                                <Link
                                                    key={subItem.name}
                                                    to={subItem.href}
                                                    className="block px-6 py-3 text-sm font-bold text-text hover:text-primary hover:bg-gray-50 transition-colors"
                                                >
                                                    {subItem.name}
                                                </Link>
                                            ))}
                ...
                <div className="flex-1 flex justify-end items-center gap-4">
                    <Link
                        to="/login"
                        className="hidden md:flex bg-primary hover:bg-primary-dark text-white px-6 py-2.5 rounded-xl font-bold text-xs lg:text-sm transition-all shadow-lg shadow-primary/20 whitespace-nowrap"
                    >
                        Post Property
                    </Link>
                    ...
                    >
                        <div className="px-6 py-6 space-y-2">
                                    ...
                                    {link.submenu ? (
                                        <>
                                            <button
                                                ...
                                                className="w-full flex justify-between items-center py-4 text-text font-bold"
                                            >
                                                {link.name}
                                                ...
                                                <div className="bg-gray-50 rounded-xl px-4 pb-2">
                                                    {link.submenu.map(sub => (
                                                        <Link
                                                            ...
                                                            className="block py-3 text-text/70 font-medium"
                                                        >
                                                            {sub.name}
                                                        </Link>
                                                    ))}
                                        ...
                                        <Link
                                            to={link.href}
                                            onClick={() => setIsOpen(false)}
                                            className="block py-4 text-text font-bold"
                                        >
                                            {link.name}
                                        </Link>
                                    )}
                            ...
                            <Link
                                to="/login"
                                onClick={() => setIsOpen(false)}
                                className="block text-center bg-primary text-white py-4 rounded-xl font-bold mt-6"
                            >
                                Post Property
                            </Link>
                        </div>
